...
class ReceiptService
{
    ...
    public function validateFile(?array $file)
    {
        ...
        if ($file['size']  > $maxFileSizeMB) {
            throw new ValidationException(['receipt' => ['File size is larger than 5MB']]);
        }
    ...
    public function upload(array $file, int $transaction)
    {
        ...
        $uploadPath = Paths::getStorageUploadsPath() . "/" . $newFilename;
        if (!move_uploaded_file($file['tmp_name'], $uploadPath)) {
            throw new ValidationException([
                'receipt' => ['Failed tp upload file']
            ]);
        }
        $this->db->query(
    ...
    {
        $filePath = Paths::getStorageUploadsPath() . "/" . $receipt['storage_filename'];
